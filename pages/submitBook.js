...
import SubmitBookContainer, { DescriptionSection } from '../components/submitBookContainer';
...
export async function getServerSideProps(context) {
    ...
    if (context.query.id) {
        ...
        existing_book = (await getBook(id)).toObject();

        delete existing_book.createdAt;
        delete existing_book.updatedAt;
        existing_book.start_date = existing_book.start_date.toISOString();
        existing_book.end_date = existing_book.end_date.toISOString();
        existing_book._id = existing_book._id.toString();
    }
    ...
    useEffect(() => {
        if (existing_book) {
            Object.keys(detailRefs).forEach(k => {
                const new_value = existing_book[k] || null;
